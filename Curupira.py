...
class AdvancedWebScanner:
    ...
    def scan_website(self, url, modules):
        ...
        try:
            ...
            # Verificar se encontramos URLs
            if not self.crawled_urls:
                print(f"{Colors.RED}[!] Nenhuma URL encontrada durante o crawling{Colors.END}")
                return False
...
def main_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            if scanner.scan_website(url, ['all']):
                input(f"\n{Colors.BLUE}>>> Pressione Enter para gerar o relatório...{Colors.END}")
            ...
            if not selected_modules:
                ...
                if scanner.scan_website(url, selected_modules):
                    input(f"\n{Colors.BLUE}>>> Pressione Enter para gerar o relatório...{Colors.END}")
                    scanner.generate_report()
